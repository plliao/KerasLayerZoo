...
class LSTMDecoder(LSTM):
    ...
    def step(self, inputs, states):
        ...
        if self.implementation == 2:
            ...
            z += K.dot(c_tm1, self.recurrent_kernel)
            if self.use_bias:
            ...
            i = self.recurrent_activation(
                x_i + K.dot(c_tm1, self.recurrent_kernel_i)
            )
            f = self.recurrent_activation(
                x_f + K.dot(c_tm1, self.recurrent_kernel_f)
            )
            c = f * c_tm1 + i * self.activation(
                x_c + K.dot(c_tm1, self.recurrent_kernel_c)
            )
            o = self.recurrent_activation(
                x_o + K.dot(c_tm1, self.recurrent_kernel_o)
            )
